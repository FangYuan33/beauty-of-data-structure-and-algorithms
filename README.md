...
AVL树的独特之处在于旋转操作（左旋，先左旋再右旋，右旋，先右旋再左旋），其可**在不影响二叉树中序遍历的前提下，使失衡节点回复平衡**，
...
相当于**穷举搜索**，穷举所有情况，然后对比得到最优解。它比较"万金油"，一般能用动态规划和贪心解决的问题它也能解决。对于需要 **"做选择"** 的问题，适合用回溯来解决。
回溯算法非常适合使用递归来实现，不过复杂度也非常高，是指数级别的，只能用来解决小规模数据问题。
可以通过**剪枝**和**备忘录**对其进行优化，**使用备忘录的回溯算法和动态规划在执行效率上基本没有区别**。

### 动态规划和贪心

动态规划一般用来解决"最优"问题。在求解的过程中能划分为多个阶段，每个阶段（子问题）对应一个状态，而下一个阶段的状态依赖当前的状态推导出来。
